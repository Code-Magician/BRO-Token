...
actor Bro {
    ...
    private var balanceOf = HashMap.HashMap<Principal, Nat>(1, Principal.equal, Principal.hash);
    if (balanceOf.size() < 1) balanceOf.put(owner, totalCoins);
    ...
    system func preupgrade() {
        balancesArr := Iter.toArray(balanceOf.entries());
    };

    system func postupgrade() {
        balanceOf := HashMap.fromIter<Principal, Nat>(
            balancesArr.vals(),
            1,
            Principal.equal,
            Principal.hash,
